...
export default function AdminTeam () {
    ...
    const [cohorts , setCohorts ] = useState([
        { title: "Spring 2025" , enrollment: 32 , status: "Starts: Mar 15, 2025", name: "Care Navigation", 
            participants: [
    ...
    if (editingCohort) {
        return (
            <EditCohort cohort={editingCohort} onClickBack={() => {
                ...
                <Flex width="100%" gap={30} direction={{ base:"column", md:"row" }}>
                    ...
                    <VStack width={{base: "100%", md:"50%"}} height="100%" border="2px solid rgb(51,51,51)" p={6} gap={4}>
                        ...
                        {teamMembers.map((member) => (
                            <EditMemberCard key={member.name} onClick={() => {setEditingCohort(null); setEditingMember(cohort);}} name={member.name} position={member.position} />
                        ))}
                    ...
                    <VStack width={{base: "100%", md:"50%"}} height="100%" border="2px solid rgb(51,51,51)"  p={6} gap={4}>
                        ...
                        {cohorts.map((cohort) => (
                            <ManageCohortCard key={cohort.title} onClick={() => {setEditingMember(null); setEditingCohort(cohort);}} title={cohort.title} enrollment={cohort.enrollment} status={cohort.status} />
                        ))}
